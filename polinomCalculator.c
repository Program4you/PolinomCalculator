...
// ((1+2x+x^2) - (1-x^3)) * ((x^2-4) - (x^3 + 7)) / (5x + 6) - (7x - 8) * (14x^2+6+x+7x^8)
// (1+x+x^2+x^3)*((x^2 - 2x + 1) - (x^2 - 3x + 2)) + ((x-5)*(x+5))

typedef struct polinom_t {
	double *coef;
	size_t n;
} polinom_t;
...
tree_t* make_tree(lexeme_t *expr, int first, int last) {
...
void print_polinom(polinom_t p) {
	for (int i = p.n; i >= 0; i--) {
		if (p.coef[i] != 0) {
			if (i < p.n)
				printf("%c", p.coef[i] > 0 ? '+' : '-');
...
polinom_t add(polinom_t p1, polinom_t p2) {
	polinom_t p;
...
polinom_t sub(polinom_t p1, polinom_t p2) {
	polinom_t p;
...
polinom_t mult(polinom_t p1, polinom_t p2) {
	polinom_t p;
	p.n = p1.n + p2.n;
	p.coef = (double *) calloc(p.n + 1, sizeof(double));
...
polinom_t divide(polinom_t p1, polinom_t p2) {
	polinom_t p;
...
polinom_t calculate(tree_t *tree) {
	if (!tree->left) {
